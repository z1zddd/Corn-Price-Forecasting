...
def train_regressor(
    ...
    *,
    ...
    """Fit a torch regressor on [N, V, T] sequence windows."""
    ...
    for _ in range(max(1, int(epochs))):
        ...
        for start in range(0, x.shape[0], batch_size):
            idx = order[start : start + batch_size]
            optimizer.zero_grad(set_to_none=True)
            loss = F.mse_loss(prediction_fn(model, x[idx]), y[idx])
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            if x_val_tensor is not None and y_val_tensor is not None and len(y_val_tensor):
                eval_loss = F.mse_loss(prediction_fn(model, x_val_tensor), y_val_tensor).item()
            else:
                eval_loss = F.mse_loss(prediction_fn(model, x), y).item()
        if eval_loss < best_loss - 1e-9:
            best_loss = eval_loss
            best_state = copy.deepcopy(model.state_dict())
            bad_epochs = 0
