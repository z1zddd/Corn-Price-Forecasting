...
DATASET_ASSETS: tuple[DatasetAsset, ...] = (
    DatasetAsset(
        ...
        commodity="corn",
        layer="compat_sample",
        path="datasets/corn_sample_data.csv",
        schema="corn_monthly_modeling_table",
        description="Small root-level CSV used by templates and quick diagnostics.",
        tracked_in_git=True,
        notes=("Root datasets/ is retained for backwards-compatible sample data paths.",),
    ),
    DatasetAsset(
        name="corn_monthly_mixed_features",
        commodity="corn",
        layer="processed",
        path="datasets/玉米价格月度_混合特征版 .csv",
        schema="corn_monthly_modeling_table",
        description="Repository-compatible monthly corn feature table with mixed price, factor, and PCA materials.",
        tracked_in_git=True,
        notes=("Existing configs may continue to reference the root datasets/ copy.",),
    ),
    DatasetAsset(
        name="corn_monthly_mixed_features_no_missing_dual_lstm",
        commodity="corn",
        layer="processed",
        path="datasets/玉米价格月度_混合特征无缺失值双头LSTM版.csv",
        schema="corn_monthly_modeling_table",
        description="Default monthly corn modeling table used by configs/corn.yaml.",
        tracked_in_git=True,
        notes=("This asset is documented in corn_forecast/datasets but not moved in this phase.",),
    ),
    DatasetAsset(
        name="corn_raw_price_source",
        commodity="corn",
        layer="raw",
        path="datasets/玉米价格原始数据.csv",
        schema="corn_monthly_modeling_table",
