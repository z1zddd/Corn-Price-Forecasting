# Docs

这里存放框架说明文档，包括架构、配置、指标、模型池、组合搜索逻辑和核心分层理念。

文档应该解释稳定的设计和使用方式，不放一次性运行记录、临时计划或实验产物。

核心设计文档：

- [Operator And Datasets Concept](operator-datasets-concept.md)
- [Architecture](architecture.md)
- [Configuration](configuration.md)
- [Official Model Pool](official-model-pool.md)
- [Leakage-Aware Ensemble Search](ensemble-search.md)
- [Corn Monthly Dataset v1](corn-monthly-v1.md)：月度核心数据、旧 PCA 隔离表、生成流程和严格回测使用规则。
- [Corn Monthly Factors v1](corn-monthly-factors-v1.md)：21 个候选月度因子的公式、时间可用性、缺失处理和数据缺口。
- [Corn Daily Factors v1](corn-daily-factors-v1.md)：30 个候选日频因子的公式、跨市场滞后、滚动窗口和数据缺口。
- [Corn Daily Market Factors v1](corn-daily-market-factors-v1.md)：由外部报价生成的 12 个短历史产业链因子、时间对齐、授权边界和残差增强用法。
