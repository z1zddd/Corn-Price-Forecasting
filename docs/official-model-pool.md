...
The registry expands the pool through `corn_forecast.operator.model.families.official.expand_model_pool`.
...
The canonical implementation is split under
`corn_forecast/operator/model/families/official/`:
...
The old `corn_forecast/modeling/specs/official/` path remains as a legacy
compatibility shim.
