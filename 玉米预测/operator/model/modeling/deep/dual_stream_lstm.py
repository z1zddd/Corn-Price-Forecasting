"""Dual-stream LSTM classifier for structured features plus news PCA inputs."""
...
class DualStreamLSTMNet(nn.Module):
    def __init__(self, structured_dim: int, news_dim: int, hidden_dim: int = 64, attn_dim: int = 32, dense_dim: int = 64, dropout: float = 0.3):
        super().__init__()
    ...
    def forward(self, structured_seq: torch.Tensor, news_seq: torch.Tensor) -> torch.Tensor:
        ...
        attn = torch.softmax(torch.matmul(q, k.transpose(1, 2)) / np.sqrt(q.shape[-1]), dim=-1)
...
class DualStreamLSTMClassifier(BaseModel):
    def __init__(
        self,
        ...
        self.news_feature_prefix = news_feature_prefix
        ...
        self.model: DualStreamLSTMNet | None = None
        self.history: dict | None = None
        self.structured_idx: list[int] = []
        self.news_idx: list[int] = []

    def fit(self, X_train, y_train, X_val=None, y_val=None) -> "DualStreamLSTMClassifier":
        ...
        self.model = DualStreamLSTMNet(
            structured_dim=len(self.structured_idx),
            news_dim=len(self.news_idx),
        ...
        for epoch in range(1, self.epochs + 1):
            ...
            for xb, yb in train_loader:
                ...
                loss = criterion(self._forward_btv(xb), yb)
                loss.backward()
                if self.grad_clip:
            ...
            self.model.eval()
            with torch.no_grad():
                val_loss = float(criterion(self._forward_btv(valid_x), valid_y).detach().cpu())
    ...
    def predict_logits(self, X) -> np.ndarray:
        ...
        with torch.no_grad():
            for start in range(0, len(x), self.batch_size):
                logits.append(self._forward_btv(x[start : start + self.batch_size]).detach().cpu().numpy())
    ...
    def load(cls, path: str | Path):
        ...
        obj._set_feature_indices()
        obj.model = DualStreamLSTMNet(
            structured_dim=len(obj.structured_idx),
            news_dim=len(obj.news_idx),
            hidden_dim=obj.hidden_dim,
            attn_dim=obj.attn_dim,
    ...
    def _forward_btv(self, x_btv: torch.Tensor) -> torch.Tensor:
        assert self.model is not None
        structured = x_btv[:, :, self.structured_idx]
        news = x_btv[:, :, self.news_idx]
        return self.model(structured, news)

    def _set_feature_indices(self) -> None:
        if not self.feature_cols:
            raise ValueError("DualStreamLSTMClassifier requires feature_cols.")
        self.news_idx = [idx for idx, col in enumerate(self.feature_cols) if col.startswith(self.news_feature_prefix)]
        self.structured_idx = [idx for idx, col in enumerate(self.feature_cols) if idx not in set(self.news_idx)]
        if not self.structured_idx:
            raise ValueError("No structured features found.")
        if not self.news_idx:
            raise ValueError(f"No news features found with prefix {self.news_feature_prefix!r}.")
    ...
    def _params(self) -> dict:
        return {
            ...
            "news_feature_prefix": self.news_feature_prefix,
            "hidden_dim": self.hidden_dim,
            "attn_dim": self.attn_dim,
            "dense_dim": self.dense_dim,
            "dropout": self.dropout,
