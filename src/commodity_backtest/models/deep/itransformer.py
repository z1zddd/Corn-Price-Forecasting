...
from commodity_backtest.models.deep.base import TorchSequenceClassifierAdapter
...
class ITransformerClassifier(nn.Module):
    ...
    def forward(self, x):
        inverted_tokens = self.variable_projection(x)
        pooled = inverted_tokens.mean(dim=1)
